...
> Обновляй при больших изменениях. Последнее обновление: **2026-06-25 — РЕАЛЬНЫЕ ШАБЛОНЫ ЗАДЕПЛОЕНЫ на prod (коммит `9bff6e9`). Договор/АВР/АО отдают исправленные seed-версии (вёрстка вычищена после визуальной сверки Саней). ДС: активна Регинина storage-версия (v2), мой seed не активен — решение по ДС открыто. Прод HEAD = `9bff6e9`.**

---

## ⚡ ВНИМАНИЕ: где сейчас стоит работа (читать перед стартом!)

# ════════ СЕССИЯ 2026-06-22 — РЕАЛЬНЫЕ ШАБЛОНЫ «ТОЧЬ-В-ТОЧЬ» (ВСЕ 4 СОБРАНЫ, ЖДУТ СВЕРКИ+ДЕПЛОЯ) ════════
...
## ✅ ВСЕ 4 ШАБЛОНА СОБРАНЫ И ПРОВАЛИДИРОВАНЫ (чекпоинт, прод по-прежнему `7623df2`, НЕ задеплоено)
- **АО** `templates/inspection-report.docx` (blip=2) — заголовок `{report.date}`, строка контакта, таблица→`{#objectServices}`, чекбоксы ручные.
- **АВР** `templates/work-completion-report.docx` (blip=2) — `build_avr()`. №/дата→`{act.number/date}`, заказчик→`{client.shortName/inn}`, контакт→`{act.responsibleName/Phone}`, дезинфектор→`{act.disinfector}`, ссылка на договор→`{contract.number/date}` (P24 собран заново чтоб не задвоить « г.»), уполномоч. лицо АВТО (append к меткам), таблица→`{#objectServices}`. «соответствует/совпадает» — статикой (руками).
- **Договор** `templates/contract-services.docx` (blip=0, картинок нет) — `build_dogovor()`. Источник АРУМ СЕРВИС. №/дата/место, преамбула Заказчика, срок→`{contract.endDate}`, Приложения №1/№2 номер+дата, реквизиты Заказчика (TABLE0 правая)→`{client.*}`, подписи (TABLE 1/3/6)→роль+`{client.shortName}`+`/{client.directorShort}/`, прайс TABLE4 (46 строк)→`{#priceItems}`. Юр.текст 1-8/Исполнитель/заявка(TABLE2)/прочие услуги(TABLE5) — статика. Заголовок прайса (P161): убран конкретный объект «Отель Hidens».
- **ДС** `templates/agreement-addendum.docx` (blip=0) — `build_ds()`. Источник ДС№2 Аппетит. №ДС→`{addendum.number}`, №договора→`{contract.number}`, даты, преамбула (точь-в-точь — без «в лице»), реквизиты+подпись. **МУЛЬТИ-ОБЪЕКТ:** P12 открывает `{#priceItemsByObject}{objectName} ({objectAddress})`, TABLE0 = вложенный `{#items}`, P13 закрывает `{/priceItemsByObject}`, TABLE1 (дубль) удалён → docxtemplater повторяет «заголовок+таблицу» на объект. Проверено на 2 объектах ✓. Колонки: Наименование→`{serviceName}`, Площадь→`{area}`, без НДС→`{priceNet}`, НДС 5%→`{vatLine}`, с НДС→`{priceGross}`, период.→`{frequency}`.
- **build-data.ts:** добавлен fallback `contract.endDate` = «31 декабря года договора» при пустом (иначе «действует по , если»).
- **tsc проверка прошла (exit 0).** Образцы для визуальной сверки: `tmp/{ao-check,avr-rendered,dogovor-rendered,ds-rendered}.docx`.

## Инструменты
- **Конвертер `tools/build-templates-from-real.py`** — `build_ao/build_avr/build_dogovor/build_ds` все РЕАЛИЗОВАНЫ + helpers (`replace_run`/`append_run`/`set_merge`/`table_to_object_cycle`/`table_to_price_cycle`/`table_to_items_cycle`/`fill_signature_cell`/`fill_requisites_cell`). Запуск: `python tools/build-templates-from-real.py ao|avr|dogovor|ds|all`.
- **Инспектор `tools/inspect-docx.py`** (НОВЫЙ) — дамп структуры .docx (параграфы/run/таблицы/blip) для карты замен. `python tools/inspect-docx.py "<file>.docx"` (нужен `$env:PYTHONIOENCODING='utf-8'`).
- **Валидатор `tools/render-test-template.js`** — рендер dot-notation парсером приложения, проверка тегов+`<a:blip>`. Тест-данные расширены 2-м объектом в `priceItemsByObject` (для мульти-объектного ДС).

## ⛔ КРИТИЧЕСКИЕ ГРАБЛИ
1. `python-docx run.text=""` УДАЛЯЕТ `<w:drawing>` — лого/QR в run'ах! `set_merge` пропускает картиночные run'ы. **ВСЕГДА** сверять `<a:blip>` в выводе == исходник (АО/АВР=2, договор/ДС=0).
2. **Реквизиты бывают 2 форматов:** договор — отдельный параграф на строку; ДС — НЕСКОЛЬКО строк в ОДНОМ параграфе через `<w:br/>`. `set_merge` на весь параграф ДС затирал соседние строки (потерял название/Почтовый/ИНН/КПП). `fill_requisites_cell` переписан ПОСТРОЧНО (split по `\n`, пересборка с `add_break()`). Универсален для обоих.
3. Чистые текстовые параграфы (без IMG/`<w:br>`) можно склеивать `set_merge`; с лого/переносами — только точечный `replace_run`.

## Решения (зафиксированы)
- **Формат прайса договора — ПЛОСКИЙ** (6 колонок АРУМ-стиля: Объект|Услуга|Площадь|Цена за 1 м²|Кратность|Адрес), решение Сани 2026-06-22. ⚠️ Реальные договоры бывают РАЗНОЙ структуры: Абрау-Ресто = таблица 48x12, мульти-объект, раздельные цены ИСТРЕБИТЕЛЬНЫЕ/ПРОФИЛАКТИЧЕСКИЕ × без/с НДС + метод. Этот формат из данных CRM (`deal_price_items` = одна цена на позицию) НЕ построить — Регина такие ведёт руками в Word. Абрау-стиль ОТЛОЖЕН на «докрут если что» (потребует расширения модели данных + второй вариативный шаблон).
- **АВР:** соответствует/совпадает — руками (статикой); уполномоч. лицо — авто.
- **ДС:** вложенный цикл по объектам; преамбула точь-в-точь (без «в лице директора», он в подписи).
- **Падежи директора** в преамбулах — именительный (как в текущих шаблонах, склонение не делали).
- **Заголовок объекта ДС** — формат «{objectName} ({objectAddress})».
- **АО чекбоксы** (◻ удовл/заселён/журнал) — ручные.

## ✅ ЗАДЕПЛОЕНО на prod (2026-06-25, коммиты `a0bd96b`→`9bff6e9`)
- **Деплой:** smoke ✓ (Ready 250ms, login 200, рестартов 0). build-data (`a0bd96b`, endDate fallback) уже был на проде в составе `a414048` (уехал непреднамеренно с combobox-деплоем 24.06).
- **`9bff6e9` — вёрстка-фиксы после визуальной сверки Саней** (set_merge схлопывал форматирование):
  - ПОДПИСИ (договор TABLE 1/3/6 + ДС): `finalize_signatures` приводит ОБЕ ячейки к `[имя/роль][пусто][пусто][подпись]` — выровнены сверху И снизу при любой длине названия (снимает `<w:br>`/`\n`, режет мусор, фикс. число пустых). Грабля: разные таблицы имели разную структуру (TABLE6 — ведущий `\n` у Исполнителя).
  - ЖИРНОСТЬ (`set_paragraph_parts`): преамбула — название жирное (договор+ДС), «Заказчик» жирный (ДС); АВР ИНН — метка жирн./значение обычн.; АВР №/дата договора жирные.
  - АВР уполномоченное: «метка: значение» вместо прочерка-табуляции (телефон не переносился).
  - ДС мульти-объект: теги цикла в ОТДЕЛЬНЫХ параграфах → заголовки всех объектов одного формата + отступ между блоками (проверено на 3 об.). `set_table_cell_margins` 120/60 twips (воздух). Договор: отступ 18pt перед «Подписи сторон».
- **Активные на проде:** contract/act_work/act_inspection = seed(файл) → отдают ИСПРАВЛЕННЫЕ. **addendum(ДС) = storage(Регина) v2 active**, мой seed v1 НЕ активен → мой ДС запечён но не отдаётся.

## ОСТАЛОСЬ
1. **РЕШЕНИЕ ПО ДС** (открыто): оставить Регинину storage-версию (v2 active) ИЛИ переключить на мой seed v1 (SQL-UPDATE is_active, обратимо). Саня решает — неизвестно что в Регининой версии.
2. **Прокликать под Региной** вживую (Claude визуал не рендерил — Word COM на машине битый, LibreOffice нет; проверка велась дампом структуры + blip + Саня смотрел скрины).
3. Источники (gitignored, локально): `DTUdocs/`, `DTU_docs_clients/Договора на заключение — копия/`. Под новым аккаунтом нужны локально.
4. Мелочи: КПП у ИП-заказчика «КПП: » пустой; `act.responsibleRole` всегда пуст. Счёт/УПД/КП — исходников нет.
5. ⚠️ `build-templates.py` для contract/act_work/addendum/act_inspection больше НЕ запускать — перезатрёт hand-crafted. Правки только через `build-templates-from-real.py`.
