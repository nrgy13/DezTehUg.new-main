...
> Обновляй при больших изменениях. Последнее обновление: **2026-06-22 — РЕАЛЬНЫЕ ШАБЛОНЫ «точь-в-точь»: ВСЕ 4 СОБРАНЫ (АО/АВР/Договор/ДС), ждут визуальной сверки Саней + деплоя. Прод HEAD = `7623df2` (не менялся; новое НЕ задеплоено).**
...
## ОСТАЛОСЬ
1. **Саня сверяет визуал в Word** 4 образцов (`tmp/*.docx`) — программно НЕ смотрел.
2. **ДЕПЛОЙ 4 шаблонов + build-data ВМЕСТЕ** (templates/*.docx — активный seed читает файл; ⚠️ `build-templates.py` для этих 4 типов больше НЕ запускать — перезатрёт hand-crafted). Бэкап не нужен (файловые), smoke после.
3. Источники (gitignored, локально): `DTUdocs/` (АО/АВР/ДС), `DTU_docs_clients/Договора на заключение — копия/` (договоры/ДС). Под новым аккаунтом нужны локально.
4. Мелочи: КПП у ИП-заказчика будет «КПП: » пустой; `act.responsibleRole` в данных пока всегда пуст. Счёт/УПД/КП — исходников нет, Саня даст отдельно.
