...
> Обновляй при больших изменениях. Последнее обновление: **2026-06-16 — большая сессия, 4 деплоя на prod (HEAD=`7623df2`): `81c707b` спринт правок (8 пунктов) → `30eb20e` поле «Метка» → `23939db` удаление документов → `7623df2` контакт в АО. + прод-правки данных техникума. Полный хендофф ниже (самодостаточный — для продолжения под любым аккаунтом).**
