...
> Обновляй при больших изменениях. Последнее обновление: **2026-06-16 (спринт правок 8 пунктов — номер/сумма договора, каскад-удаление, поиск календаря, наряды, навигация — ЗАДЕПЛОЕН на prod, коммит 81c707b).**

---

## ⚡ ВНИМАНИЕ: где сейчас стоит работа (читать перед стартом!)

**Спринт правок (8 пунктов) ЗАДЕПЛОЕН на prod 2026-06-16** (коммит `81c707b`, ветка feature/crm, БЕЗ миграций). Бэкап `prod-db-20260616-before-ten-fixes.sql.gz` (100K). Smoke ✅ (Ready 303ms, login 200, manager 307, manifest 200, рестартов 0). ⚠️ В начале сессии прод ЛЕЖАЛ (Beget остановил VPS — SSH+HTTPS timeout; ожил через ~10 мин, uptime 6 min при деплое — баланс/перезапуск Beget, НЕ firewall). **Сделано:** (1) номер договора правится в Реквизитах (`DealRequisitesTab`+`updateDeal`+`dealFormSchema`, опционален чтобы createDeal не сломать; аудит from→to; офиц. номер `ДГ-2026-NNN` независим от `contractNumber`); (2) **каскадное удаление клиента** (Саня выбрал каскад вместо мягкого запрета) — `deleteClient(id,{force})` двухшаговый: needsConfirm со списком зависимостей → подтверждение → транзакция (deals каскад→price/addendums/worklogs, client каскад→objects/services, leads SET NULL), документы (файлы+записи) удаляются явно, полный снимок в `activity_log`; (3) Аппетит-форма объекта: ошибки строк услуг видимы + `toast` всегда (фикс «молча не сохраняется») + `.positive()`→`.nonnegative()`; (4) поиск календаря `CalendarFull`: +masterName +услуга в haystack, при вводе показ ВСЕХ совпадений (не только видимый месяц); (7+8) **«Сумма договора» = ручное поле** `total_amount` (createDeal не трогает) + «Итого по прайсу» (Σ price_with_vat) рядом — у годовых договоров суммы РАЗНЫЕ (НМТП: договор 80000 vs прайс 10000), арифм. бага НЕ было, расходился только список(stored)↔карточка(Σ); (9) `getObjectWorkOrderDefaults` 3-й fallback: услуги из `deal_price_items` по объекту когда `client_object_services` пуст (+дедуп) — на проде реальные услуги в прайсе, объектные разрежены; (10) хлебные крошки `components/crm/Breadcrumbs.tsx` в 3 карточках (клиент/договор/объект), табы разведены («Все объекты»/«Объекты договора»), объект кликабелен в `VisitsTab`. Импортёр `parse-regina-contracts.py` +дедуп прайс-позиций. ⚠️ **Вживую под Региной НЕ кликал** (автономный деплой). Подробности: `memory/ten_fixes_deployed.md`.
- **ОТЛОЖЕНО:** п.5 (техникум — «дубль при выборе объекта» в прайсе) и п.6 (техникум — кнопка привязки объекта) — код корректен, ждут ТОЧНОГО симптома от Сани («Славянск на Кубани» в БД нет = это Славянский сельхозтехникум `b369d927`). **⛔ «Чистка 58 дублей прайса» ОТМЕНЕНА — это НЕ дубли, а СЕТЕВЫЕ клиенты** (прайс по каждой точке): Гончаров = сеть «Хадыжи» 31 магазин/31 строка, Устименко 10/10, Сухарникова 10 точек/20 строк и т.д. Суммы ВЕРНЫЕ, удалять НЕЛЬЗЯ. Подтверждает «Сумма договора = ручное поле». Также 2026-06-16 на проде: объекты техникума `b369d927` привязаны к договору 266/25-К + имена почищены (убран префикс учреждения). Поле «Тип объекта» в АО/АВР НЕ выводится (проверено по плейсхолдерам) — обсуждается переназначить под различитель объектов сетей. Детали: `memory/ten_fixes_deployed.md`.
