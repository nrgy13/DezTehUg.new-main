...
- **ОТЛОЖЕНО:** п.5 (техникум — «дубль при выборе объекта» в прайсе) и п.6 (техникум — кнопка привязки объекта) — код корректен, ждут ТОЧНОГО симптома от Сани («Славянск на Кубани» в БД нет = это Славянский сельхозтехникум `b369d927`). **Чистка 58 дублей прайса** (Гончаров 31×→1, Устименко 10×→1, Сухарникова, Колисов, Свистунова, Ярошенко — импортные дубли, раздули суммы) — необратимый DELETE, Саня сказал «дубли позже», превью готово (см. memory).
