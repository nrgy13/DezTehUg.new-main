...
> Обновляй при больших изменениях. Последнее обновление: **2026-06-09 (Чеклист для мастера в форме наряда + аудит-фиксы 3 модулей ЗАДЕПЛОЕНЫ на prod — автономная ночная сессия).**

---

## ⚡ ВНИМАНИЕ: где сейчас стоит работа (читать перед стартом!)

**Чеклист для мастера в форме наряда + аудит-фиксы ЗАДЕПЛОЕНЫ на prod 2026-06-09** (коммиты `7250525`/`7089f64`/`eb69eba`, БЕЗ миграций). Автономная ночная сессия. ФИЧА: Регина собирает чеклист мастеру в `WorkOrderDialog` (автоподтяг из прошлого наряда/шаблонов услуг, ручные пункты, добавить-заменить, «обязательный»). АУДИТ 3 субагентами → 32 находки, пофикшено 10 (роль в getActor — master мог CRUD объекты!; bodySizeLimit 6mb — фото мастера было нерабочим; транзакции createWorkOrder/addObject; атомарный finalize; статус-гарды; дубль-защита наряда). Бэкап `prod-db-20260609-0407-before-night-fixes.sql.gz`. Отложенные улучшения: `tmp/plans/improvements-backlog.md` (TZ напоминаний, семантика required, история объектов, MIME фото — НЕ критичны). Подробности: `memory/checklist_and_audit_fixes.md`.

**➡️ СЛЕДУЮЩЕЕ — спринт полировки (план готов, развилки ОТВЕЧЕНЫ, реализация с НОВОЙ сессии):** `tmp/plans/polish-sprint.md`. Порядок **P1→P2→P3→P4**: **P1** заголовок выезда «Без услуги» у заказ-нарядов (видимый баг, мастер видит на каждом наряде — первым); **P2** TZ гашения напоминаний (НЕ срочно — Регина периодичность не проставила, но окно идеальное пока цикл спит); **P3** семантика обязательных пунктов чеклиста — **Саня выбрал вариант A**: `na` на обязательном требует заметку-обоснование (мастер вправе пометить «неприменимо», но обязан объяснить); **P4** чистка (push мастеру о новом наряде — сейчас НЕ уведомляется!, мёртвый код, дедуп пунктов). НЕ трогать перфекционизм без отдачи при масштабе «Регина+1 мастер» (TOCTOU, MIME magic-bytes, computeDiff).
