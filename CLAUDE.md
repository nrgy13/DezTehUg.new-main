...
**➡️ СЛЕДУЮЩЕЕ — спринт полировки (план готов, реализация с НОВОЙ сессии):** `tmp/plans/polish-sprint.md`. Приоритеты (мнение Claude): **P1** заголовок выезда «Без услуги» у заказ-нарядов (видимый баг — мастер видит его на каждом наряде, делать первым); **P2** TZ гашения напоминаний (превентивно ДО включения цикла Региной — иначе флажки заглючат на старте); **P3** семантика «обязательных» пунктов чеклиста (⚠️ нужно решение Сани: `na` обходит обязательность — A мягко/B жёстко/C убрать блок); **P4** чистка (push мастеру о новом наряде — сейчас НЕ уведомляется!, мёртвый код, дедуп пунктов). НЕ трогать перфекционизм без отдачи при масштабе «Регина+1 мастер» (TOCTOU, MIME magic-bytes, computeDiff и пр.). Развилки для Сани — в шапке плана.
